...
describe('codeplug format version — read path', () => {
  ...
  it('opens a tables file from before formatVersion without a newer-format warning', () => {
    // Builds before formatVersion wrote `tables` files with only
    // `version: '1.1.0'`. A reader at format 1.0.0 would warn on every one.
    const data = jsonSafeToCodeplug({ version: '1.1.0', tables: {} });
...
describe('assertReadableFormat', () => {
  it('allows equal and older versions', () => {
    expect(() => assertReadableFormat(CODEPLUG_FORMAT_VERSION)).not.toThrow();
...
describe('checkFormatCompatibility', () => {
  it('classifies each direction', () => {
    expect(checkFormatCompatibility('1.1.0')).toBe('ok');
    expect(checkFormatCompatibility('1.0.0')).toBe('ok');
    expect(checkFormatCompatibility('0.9.0')).toBe('ok');
    expect(checkFormatCompatibility('1.1.9')).toBe('ok'); // patch never matters
    expect(checkFormatCompatibility('1.2.0')).toBe('newer-minor');
