...
| ❌ | **Talk group locator** | **FAILED ON THE RADIO 2026-09-10.** Deleting `TG0501` (slot 500) wrote exactly what was intended — mask bit clear, `locator[500]` = `ff ff ff ff`, `locator[501]` = 501 not 500, all survivors on their own slots — and the **read-back matched perfectly**. Then the radio reported **1010** talk groups and **crashed** when navigated to the deleted one. `planSpanTableWrite` copies the original into the gap, so the RECORD went back fully populated; the radio's list does not come from the mask. **A clean read-back is not a pass** — it proves our decoder agrees with our encoder and nothing about what the radio does with the result. Add/delete is refused in `d890WriteInput.ts` until the vendor CPS shows what a delete actually writes. |
