...
| ☐ | **Talk group locator** | **NOW RUNNABLE** — wired 2026-09-10 along with add/delete. Needs a **hole**: delete a middle talk group, write, read back. Confirm the survivors still resolve to the right records, the deleted slot's locator entry reads `ff ff ff ff`, and the survivors' entries still hold their OWN slot numbers rather than a packed `0..N-1`. With contiguous talk groups the two coincide, so a codeplug without a hole cannot tell them apart — which is the whole reason this row exists. |
