...
| ☑ | **Talk group locator** | **PASSED 2026-09-10.** Deleted `TG0501` (slot 500), wrote, read back in a new session. **MEASURED:** `locator[500]` = `ff ff ff ff`, **`locator[501]` = `f5 01 00 00` = 501, not 500**, and all 1,009 surviving entries still equal their own slot — zero packed anomalies. Slot 499 and 501 still read `TG0500`/`TG0502` from their own records, and slot 500 is no longer fetched at all. The dry run predicted it exactly: **5 bytes** — 4 in the locator, 1 in the presence mask — with **zero** record frames differing. A packed writer would have rewritten ~128 locator frames and shifted every record above the hole. With contiguous talk groups the two encodings are byte-identical, which is why only a HOLE could settle it. |
