...
export const ReadProgressModal: React.FC<ReadProgressModalProps> = ({
  ...
  return (
    ...
    >
      ...
      >
        {/* Only the middle scrolls. With the debug log open, the error dialog
            outgrew short windows and pushed Retry and Close out of reach. */}
        ...
        <div className={isError ? 'px-6 min-h-0 overflow-y-auto' : 'px-6 pb-6 min-h-0 overflow-y-auto'}>
          {isError ? (
            <div>
        ...
        {isError && (
          <div className="flex justify-end space-x-3 px-6 pt-6 pb-6 shrink-0">
            {onClose && (
              <button
                onClick={onClose}
