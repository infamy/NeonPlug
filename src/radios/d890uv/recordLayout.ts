...
export const D890_MEMORY_MAP: readonly D890Region[] = [
  ...
  { name: 'Channel mask', address: D890_ADDR.CHANNEL_SET, size: D890_ADDR.CHANNEL_SET_SIZE,
    ...
    note: 'ROUND-TRIPPED 2026-09-11: channel 102 was deleted in the Write A batch and the vendor CPS, reading the radio afterwards, no longer listed it — with channel 101 still present, so the write cleared one bit and left its neighbour alone. Not checkable on the radio itself: 102 belonged to no zone, and this radio browses channels through zones.' },
  ...
  { name: 'Settings', address: D890_ADDR.SETTINGS, size: D890_ADDR.SETTINGS_SIZE,
    ...
    note: 'ROUND-TRIPPED 2026-09-12: keyTone 0x000 off -> on and sqlLevelA 0x009 2 -> 5, written together as two bytes in one frame. Confirmed on the radio itself rather than by read-back — a key press beeped where it had been silent, and the squelch menu read 5. The dry run accounted for exactly those two bytes and tot at 0x004 was still 4 afterwards, so nothing else in the region moved.' },
  ...
  { name: 'Power-on display', address: 0x3500900, size: 0x60,
    ...
    read: true, write: true, hardwareRoundTrip: true, provenance: 'hardware',
    note: 'ROUND-TRIPPED 2026-09-11: line 1 written WELCOME -> ZULU ONE, and the vendor CPS reading the radio showed ZULU ONE with line 2 still ANYTONE — only the line that was written moved. Not visible on the radio itself while powerOnInterface is 2 (Custom Picture): the boot screen draws the image, so the text is checkable in the CPS or after switching that setting to 1 (Custom Char). CONFIRMED 2026-08-31: the CPS Power-on tab showed WELCOME / ANYTONE / 12345678 and the dump reads exactly that. Note the MIXED encoding — the two text lines are UTF-16LE, the password is plain ASCII. Fields are 0x20 each but the vendor declares varchar(14), and the CPS draws 14 boxes, so only 14 characters are decoded.' },
  ...
  { name: 'Talkgroups', address: D890_ADDR.TALKGROUP_DATA, stride: D890_ADDR.TALKGROUP_STRIDE,
    ...
    read: true, write: true, hardwareRoundTrip: true, provenance: 'hardware',
    note: 'ADD AND DELETE ROUND-TRIPPED ON HARDWARE 2026-09-11. On a radio holding TG0001-TG0020, one write deleted TG0005 and TG0010 and added RT Zulu TG: the list compacted to 19, the new record was built on zeros, and the freed slot was written the vendor\'s way (a zero tail in the frame it shares, the rest erased). The radio listed TG0001-0004, 0006-0009, 0011-0020 and RT Zulu TG last, and did not crash at the end of the list. EDIT round-tripped 2026-09-09 in bank 1 (slot 1004), which proved the banking. This note used to say only six talkgroups had ever been loaded, which predates the 1,010-talk-group radio.' },
  ...
  { name: 'VFO A / VFO B', address: 0x1f81000, stride: 0x80,
    ...
    read: true, write: true, hardwareRoundTrip: true, provenance: 'hardware',
    // ROUND-TRIPPED 2026-09-11: VFO A written 435.06250 -> 438.73750 and the
    // radio displayed it. Check the screen BEFORE turning the dial — this is
    // live operator state and the knob overwrites it.
  ...
  { name: 'Pre-defined SMS', address: 0x3180000, stride: 0x200,
    ...
    read: true, write: true, hardwareRoundTrip: true, provenance: 'hardware',
    note: 'WIRED FOR WRITE 2026-09-11 — edit, add and delete — together with the SMS store chain at 0x2980000, because THE CHAIN IS THE LIST: the radio and the vendor CPS walk it from its head and fetch each envelope\'s text slot, and a text no envelope names is not a message. MEASURED from CPS captures: on 2026-09-10 the CPS dropped "Thank you!", whose envelope had been retired while its text still sat in slot 2, so the reader now follows the chain instead of scanning text slots (which would have listed it, and a write would have put it back). The CPS writes each text as the text, a NUL and zeros to 0x200 — our encoder reproduces all five factory records byte for byte (tests/fixtures/d890uv/sms-vendor-texts.bin) — and each envelope as 00 00 next textSlot then twelve zeros. DELETE keeps slots, as the radio\'s own delete does (2026-09-08: envelope retired, text erased, survivors untouched); the CPS instead COMPACTS on every write (7x2_onecleared.txt rewrote three survivors into slots 0-2), and the radio reads both. Keeping slots keeps hot keys, whose Content byte names a TEXT slot, on the message they were set to, and a delete that would strand one is refused. ADD takes the lowest slot nothing uses, a hot key\'s included. ROUND-TRIPPED ON HARDWARE the same day, on the radio\'s own quick-text list: from Welcome!/Good bye!/Happy every day! in slots 0-2, one write added RT Zulu 42 (slot 3), deleted Good bye! (slot 1: text erased, envelope retired) and edited Welcome! to Welcome Zulu — 1,048 bytes in 70 frames, exactly the dry run — and the radio listed Welcome Zulu, Happy every day!, RT Zulu 42. So the radio follows the chain across a hole, takes each text from the slot its envelope names, and accepts an envelope built from zeros. CONFIRMED: returned the five AnyTone factory defaults. Text is UTF-16LE, NOT the varchar(200) the vendor SQL DDL implies — that DDL describes the CPS database, not the radio.' },
  ...
  { name: 'FM broadcast channels', address: 0x3400000, stride: 0x40, read: true, write: true,
    ...
    hardwareRoundTrip: true, provenance: 'hardware',
    // ROUND-TRIPPED 2026-09-11: FM-001 at 108.0 rewritten as ZULU FM at 101.9,
    // and the radio's FM channel list showed both the name and the frequency.
  ...
  { name: 'FM VFO', address: 0x3402000, size: 0x60,
    ...
    read: true, write: true, hardwareRoundTrip: true, provenance: 'hardware',
    // ROUND-TRIPPED 2026-09-11: written 108.0 -> 99.5 and the radio's FM VFO
    // tuned there.
  ...
  { name: 'AM airband channels', address: 0x3880000, stride: 0x40, read: true, write: true,
    ...
    hardwareRoundTrip: true, provenance: 'hardware',
    // ROUND-TRIPPED 2026-09-11: AM index 0 (CZBB TWR, 118.100) rewritten as
    // ZULU AM at 119.300, and the first channel of airband zone CZBB read
    // 119.300. The airband screen shows frequencies rather than names, which is
    // why the frequency was changed too — a rename alone is unverifiable there.
  ...
  { name: 'FM broadcast scan mask', address: 0x3402050, size: 0x10,
    ...
    read: true, write: true, provenance: 'hardware',
    note: 'Flat, unlike AM\'s, which is a u32 bitmap per zone over member POSITIONS. An FM channel therefore has exactly one scan state and belongs as a column on the channel table; an AM channel\'s depends on which zone is being scanned. Read into scanAdd since the driver started but NOT written until 2026-09-03 — and the address sits inside the verbatim preserve run at 0x3402000, so an edit was actively overwritten with its pre-edit bytes on every write. Planning it explicitly is what stops that. ⚠️ UNRESOLVED 2026-09-11: a write that cleared bit 0 (FM index 0, scanAdd true -> false, one byte, sent with no refusal) left the vendor CPS still showing that channel as Add. Polarity is not the suspect — the vendor CPS\'s own read capture holds 01 at this address, and both our read and our write take set = scanned. Either the CPS column is fed from somewhere else or the write to this address does not stick; reading 0x3402050 back through NeonPlug separates the two. MEASURED 2026-09-11: the radio still holds 01 after a write of 00, so the write does NOT stick here — see HW-ROUNDTRIP-TESTS.md. Left read/write because the encoder and address are right; the radio appears to own this byte at runtime. Never mark it round-tripped on a clean dry run alone.' },
  ...
  { name: 'AM airband zones', address: 0x3888000, stride: 0x80,
    ...
    read: true, write: true, hardwareRoundTrip: true, provenance: 'hardware',
    // ROUND-TRIPPED 2026-09-11: zone 2 renamed VYVR -> ZULU ZONE and the radio's
    // airband zone list showed it. Its members and CurWorkCH were untouched: the
    // zone still opened on AM index 6 (CYVR TWR, 118.700).
