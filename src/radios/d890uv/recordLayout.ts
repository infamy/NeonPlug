...
export const D890_MEMORY_MAP: readonly D890Region[] = [
  ...
  { name: 'Encryption IDs', address: 0x3585000, stride: 2,
    ...
    read: false, provenance: 'hardware',
    note: 'CONFIRMED: read 01 01 02 02 … 20 20, the factory IDs 1-32, matching the prediction before the dump. Address came from the vendor marshaller, not from searching — this table is nowhere near the regions it was hunted for in.' },
  { name: 'Encryption keys', address: 0x3585100, stride: 0x28,
    contents: '32 slots; only +0x10/+0x11 hold the 16-bit key, big-endian',
    read: false, provenance: 'marshaller',
    note: 'The key passes through XOR with a mask that is 0 unless the CPS has an activation file loaded. Bytes +0x00-0x0f and +0x12-0x27 of each slot are unresolved.' },
  { name: 'AES encryption keys', address: 0x3580000, stride: 0x40,
    contents: 'key_id at +0x00, key bytes from +0x01, aes_key_num at +0x22',
    read: false, provenance: 'marshaller',
    note: 'NOT confirmed. Read all zeros, which is consistent with every slot at factory default AND with the address being wrong — unused memory reads zeros too. Needs a codeplug with a key actually set.' },
