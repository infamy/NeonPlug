...
export const D890_MEMORY_MAP: readonly D890Region[] = [
  { name: 'Device identity', address: 0x07000000, size: 16, contents: 'Model string returned by the 0x02 probe',
    read: true, provenance: 'hardware', note: 'NEVER WRITE. The model string the 0x02 probe returns is the radio identifying itself; writing here would change what the radio claims to be. Never write here.' },
  ...
  { name: 'Zone membership', address: D890_ADDR.ZONE_CHANNELS, stride: D890_ADDR.ZONE_CHANNELS_STRIDE,
    contents: `250 × u16 zero-based channel indices, 0xffff-terminated`, read: true, write: true, provenance: 'hardware',
  ...
  { name: 'Zone present mask', address: D890_ADDR.ZONE_SET, size: D890_ADDR.ZONE_SET_SIZE,
    contents: 'One bit per zone; SET = present', read: true, write: true, provenance: 'hardware' },
  ...
  { name: 'Master radio ID', address: D890_ADDR.MASTER_ID_DATA, size: D890_ADDR.MASTER_ID_SIZE,
    contents: 'The radio’s own ID record', read: false, provenance: 'hardware',
    note: 'Read flag CORRECTED 2026-09-01 from true to false. Nothing references MASTER_ID_DATA outside this map — no protocol method reads it and no parser exists. Found while closing the read/write gap, because it was the only readable-but-not-encodable region left that was not deliberate. The vendor CPS writes 64 bytes here, so the region is real; NeonPlug simply does not read it yet.' },
  ...
  { name: 'Boot image', address: 0x03f80000, size: 40960,
    contents: '160x128 RGB565, big-endian, column-major, no header', read: true, optional: true, write: true, provenance: 'hardware',
  ...
  { name: 'Background image 1', address: 0x04000000, size: 40960, contents: 'Same format as the boot image',
    read: true, optional: true, write: true, provenance: 'hardware',
  ...
  { name: 'GPS Roaming / zone bars', address: 0x3502000, stride: 0x20, read: true, write: true,
