...
export const D890_MEMORY_MAP: readonly D890Region[] = [
  ...
  { name: 'Boot image', address: 0x03f80000, size: 40960,
    contents: '160x128 RGB565, big-endian, column-major, no header', read: false, provenance: 'hardware',
  ...
  { name: 'Roaming channel bitmap', address: 0x2084000, size: 0x20,
    contents: 'One bit per roaming channel; SET = present', read: false, provenance: 'hardware',
  ...
  { name: 'FM broadcast channels', address: 0x3400000, stride: 0x40,
  ...
  { name: 'FM broadcast (second block)', address: 0x3402000, size: 0x60,
    contents: 'Holds a record shaped like the FM channels, name beginning "VF" — probably the FM VFO',
    read: false, provenance: 'inferred' },
  { name: 'AM airband channels', address: 0x3880000, stride: 0x40,
  ...
  { name: 'AM airband masks', address: 0x3884000, size: 0x40,
    ...
    read: false, provenance: 'marshaller',
    note: 'mapping.md calls this "VFO". It is not — see the VFO entry. Three read attempts returned nothing, so even the mask role is unverified.' },
  { name: 'AM airband zones', address: 0x3888000, stride: 0x80,
    contents: 'Zones over the AM airband channel table, separate from the main zone list',
    read: false, provenance: 'marshaller',
    note: 'Nothing is set in the captured codeplug, so this cannot be mapped by dumping — it needs the vendor disassembly.' },
  ...
  { name: 'Emergency / alarm (1)', address: 0x3482e00, size: 0x30,
  ...
  { name: 'Emergency / alarm (2)', address: 0x3483000, size: 0x30,
  ...
  { name: '5-Tone', address: 0x3480000, stride: 0x20,
    contents: '5-Tone encode/decode', read: false, provenance: 'marshaller' },
  { name: '2-Tone', address: 0x3482000, stride: 0x20,
    contents: '2-Tone encode/decode', read: false, provenance: 'marshaller' },
  ...
  { name: 'ARC4 encryption keys', address: 0x3584000, stride: 0x10,
    contents: 'key_id at +0x00, then 5 key bytes', read: false, provenance: 'marshaller',
    note: 'NOT confirmed, same reason as the AES table.' },
  { name: 'Unidentified table', address: 0x3502000, stride: 0x20,
    contents: 'Records of 0x20 bytes, each reading 00 FF then zeros in this codeplug',
    read: false, provenance: 'unknown',
    note: 'Read off the radio but not identified. A 32-entry table of this stride would fit the 32-slot encryption-code table, but the bytes do not match its known contents (keys all zero, IDs 0x0101..0x2020), so nothing is claimed.' },
