...
export const D890_MEMORY_MAP: readonly D890Region[] = [
  ...
  { name: 'Zone hidden mask', address: D890_ADDR.ZONE_HIDE, size: D890_ADDR.ZONE_HIDE_SIZE,
    contents: 'One bit per zone; SET = hidden from the radio menu', read: true, provenance: 'marshaller',
    note: 'CONFIRMED as a real DA-7X2 feature. It is reachable two ways in the vendor CPS, both obscure: per zone in the zone edit dialog, and in bulk via Tool > Zone Hide Operation (All Checked / All UNChecked). The owner describes it as very buried and not clear what it does — which is why it reads as absent from the Zone grid, where there is no hide column. NeonPlug surfaces the per-zone flag directly in the zone editor beside the A/B channels rather than reproducing that.' },
